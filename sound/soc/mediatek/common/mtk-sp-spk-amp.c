...
// TN Begin modify To Differentiate between YC and QC speakers. by lianjie.chen 20231117 TIAIAA-8.
#include <linux/of_gpio.h>
// TN End modify To Differentiate between YC and QC speakers. by lianjie.chen 20231117 TIAIAA-8.
...
// TN Begin modify To Differentiate between YC and QC speakers. by lianjie.chen 20231117 TIAIAA-8.
static int spk_boxid[MAX_BOX_NUM] = {0, 0, 0, 0};
static int spk_gpio[MAX_BOX_NUM];
// TN End modify To Differentiate between YC and QC speakers. by lianjie.chen 20231117 TIAIAA-8.
...
// TN Begin modify To Differentiate between YC and QC speakers. by lianjie.chen 20231117 TIAIAA-8.
int mtk_spk_get_boxid(void)
{
	if (spk_boxid[0] == 0 && spk_boxid[1] == 0 && spk_boxid[2] == 0 && spk_boxid[3] == 0) {
		return MTK_BOX_ALL_QS;
	} else if (spk_boxid[0] == 1 && spk_boxid[1] == 1 && spk_boxid[2] == 1 && spk_boxid[3] == 1) {
		return MTK_BOX_ALL_YC;
	} else {
		return MTK_BOX_QS_YC;
	}
}
EXPORT_SYMBOL(mtk_spk_get_boxid);

int mtk_spk_update_boxid(struct platform_device *pdev)
{
	int ret, i;

	spk_gpio[0] = of_get_named_gpio(pdev->dev.of_node, "spkid0-gpio", 0);
	if (spk_gpio[0] < 0) {
		dev_err(&pdev->dev, "no spkid0 gpio provided");
	}

	spk_gpio[1] = of_get_named_gpio(pdev->dev.of_node, "spkid1-gpio", 0);
	if (spk_gpio[1] < 0) {
		dev_err(&pdev->dev, "no spkid1 gpio provided");
	}

	spk_gpio[2] = of_get_named_gpio(pdev->dev.of_node, "spkid2-gpio", 0);
	if (spk_gpio[2] < 0) {
		dev_err(&pdev->dev, "no spkid2 gpio provided");
	}

	spk_gpio[3] = of_get_named_gpio(pdev->dev.of_node, "spkid3-gpio", 0);
	if (spk_gpio[3] < 0) {
		dev_err(&pdev->dev, "no spkid3 gpio provided");
	}

	if (gpio_is_valid(spk_gpio[0])) {
		ret = devm_gpio_request_one(&pdev->dev, spk_gpio[0],
			GPIOF_DIR_IN, "spkid0");
		if (ret) {
			dev_err(&pdev->dev, "spkid0 request failed");
		}
	}

	if (gpio_is_valid(spk_gpio[1])) {
		ret = devm_gpio_request_one(&pdev->dev, spk_gpio[1],
			GPIOF_DIR_IN, "spkid1");
		if (ret) {
			dev_err(&pdev->dev, "spkid1 request failed");
		}
	}

	if (gpio_is_valid(spk_gpio[2])) {
		ret = devm_gpio_request_one(&pdev->dev, spk_gpio[2],
			GPIOF_DIR_IN, "spkid2");
		if (ret) {
			dev_err(&pdev->dev, "spkid2 request failed");
		}
	}

	if (gpio_is_valid(spk_gpio[3])) {
		ret = devm_gpio_request_one(&pdev->dev, spk_gpio[3],
			GPIOF_DIR_IN, "spkid3");
		if (ret) {
			dev_err(&pdev->dev, "spkid3 request failed");
		}
	}

	for (i = 0; i < MAX_BOX_NUM; i++) {
		spk_boxid[i] = gpio_get_value(spk_gpio[i]);
		dev_err(&pdev->dev, "gpio_get_value spk_boxid%d  %d", i, spk_boxid[i]);
	}

	return 0;
}
EXPORT_SYMBOL(mtk_spk_update_boxid);
// TN End modify To Differentiate between YC and QC speakers. by lianjie.chen 20231117 TIAIAA-8.
