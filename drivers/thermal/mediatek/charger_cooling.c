...
//TN add begin for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
#include "mtk_charger.h"
//TN add end for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
#include "charger_cooling.h"

//TS add begin for monitoring LCD status to apply corresponding thermal policy CALLA-609 by lgf 20231204
#include "mtk_disp_notify.h"
static int lcm_status =0;
static int lcm_status_switched = 1;//TN add to indicate if the lcm status have changed since last set cooling state OCALLA-817 by lgf 20231225
//TS add begin for monitoring LCD status to apply corresponding thermal policy CALLA-609 by lgf 20231204
struct charger_cooler_info {
	int num_state;
	int cur_current;
	int cur_state;
        int charger_type;////TN add for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
};

static struct charger_cooler_info charger_cl_data;
/* < -1 is unlimit, unit is uA. */
//TS modify begin for tuning the thermal policy CALLA-609 by lgf 20231204
static const int master_charger_state_to_current_limit[CHARGER_STATE_NUM] = {
	-1, 6000000, 5000000, 4000000, 3000000, 3000000, 3000000, 3000000, 3000000
};
static const int master_charger_state_to_current_limit_qc[CHARGER_STATE_NUM] = {
        -1, 3000000, 2000000, 1500000,1000000,0,0,0,0
 };

//TS modify end  for tuning ther thermal policy CALLA-609 by lgf 20231204
...
static int charger_throttle(struct charger_cooling_device *charger_cdev, unsigned long state)
{
	...
	charger_cl_data.charger_type = mtk_chg_get_charger_type();//TN add for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
	...
	//TN modify begin for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
	if(charger_cl_data.charger_type == CHARGER_TYPE_PPS)
		charger_cl_data.cur_current = master_charger_state_to_current_limit[state];
	else if(charger_cl_data.charger_type == CHARGER_TYPE_QC)
		charger_cl_data.cur_current = master_charger_state_to_current_limit_qc[state];
	else
		charger_cl_data.cur_current = -1;
	//TN modify end for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
	dev_info(dev, "%s: set lv = %ld done, charger_type=%d\n", charger_cdev->name, state,charger_cl_data.charger_type);
	return 0;
}
...
static int charger_cooling_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if(lcm_status == 0)
		return 0;
	//TS add end for monitoring LCD status to apply corresponding thermal policy CALLA-609 by lgf 20231204

	//TN add begin for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
	charger_type = mtk_chg_get_charger_type();
	pr_info("charger_cooling_set_cur_state begin charger_type=%d,charger_cl_data.charger_type=%d,state=%d\n",charger_type,charger_cl_data.charger_type,state);
	//TN add end  for apply different thermal policy  according to charger type CALLA-736 by lgf 20231221
...
static int cooling_state_to_charger_limit_v1(struct charger_cooling_device *chg)
{
	...
	if (charger_cl_data.charger_type == CHARGER_TYPE_PPS)
		...
		prop_bat_chr.intval = master_charger_state_to_current_limit_qc[chg->target_state];
	else
	...
	if (chg->type ==THERMAL_DUAL_CHARGER) {
...
//TN modify the functin name to avoid duplicated define with charger module OCALL-736 by lgf 20231221
