...
static int mtk_mipi_tx_pll_prepare_mt6789(struct clk_hw *hw)
{
	...
	if (mipi_tx == NULL)
		return -EINVAL;
	DDPDBG("%s+\n", __func__);
	// TN modified begin by chen.yan/860607 20231116 CR/OCALLA-652
	/*set volate*/
	writel(0x44442AEA, mipi_tx->regs + MIPITX_VOLTAGE_SEL);
	// TN modified end by chen.yan/860607 20231116 CR/OCALLA-652
...
// TN modified begin by chen.yan/860607 20231116 CR/OCALLA-333
	rate = (mipi_tx->data_rate_adpt) ? mipi_tx->data_rate_adpt :
			mipi_tx->data_rate / 1000000;
	rate_khz = (mipi_tx->data_rate_khz_adpt) ? mipi_tx->data_rate_khz_adpt : mipi_tx->data_rate / 1000;
	pr_notice("%s: prepare: %u KHz\n", __func__, rate_khz);
// TN modified end by chen.yan/860607 20231116 CR/OCALLA-333
	DDPINFO(
