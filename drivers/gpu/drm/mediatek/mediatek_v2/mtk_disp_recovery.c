...
// TN modified by kexin.wang/860557 20240129 CR/OCALLA-2675
...
// TN modified begin by chen.yan/860607 20231116 CR/OCALLA-653
void (*nvt_lcd_esd_resume)(bool);
EXPORT_SYMBOL_GPL(nvt_lcd_esd_resume);
// TN modified end by chen.yan/860607 20231116 CR/OCALLA-653
...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		do {
			...
			// TN modified begin by chen.yan/860607 20231116 CR/OCALLA-653
			if(nvt_lcd_esd_resume)
			nvt_lcd_esd_resume(false);
			// TN modified end by chen.yan/860607 20231116 CR/OCALLA-653
			mtk_drm_esd_recover(crtc);
			// TN modified begin by chen.yan/860607 20231116 CR/OCALLA-653
			if(nvt_lcd_esd_resume)
			nvt_lcd_esd_resume(true);
			// TN modified end by chen.yan/860607 20231116 CR/OCALLA-653
		...
		/* TN Begin modified by kexin.wang/860557 20240129 CR/OCALLA-2675*/
		if (need_setbacklight) {
			DDPPR_ERR("%s: wkx: need_setbacklight\n", __func__);
			mtkfb_esd_rec_set_backlight_level();
			need_setbacklight = 0;
		}
		/* TN end modified by kexin.wang/860557 20240129 CR/OCALLA-2675*/
