...
static int32_t aw_sar_input_init_comm(struct aw_sar *p_sar)
{
	...
	for (i = 0; i < p_sar->p_sar_para->ch_num_max; i++) {
		...
		if ((p_sar->dts_info.channel_use_flag >> i) & 0x01) {
			...
			__set_bit(KEY_SAR_FAR, p_sar->channels_arr[i].input->keybit);
			__set_bit(KEY_SAR_NEAR, p_sar->channels_arr[i].input->keybit);
			__set_bit(KEY_SAR_CLOSE, p_sar->channels_arr[i].input->keybit);
