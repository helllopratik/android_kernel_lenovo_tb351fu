				...
				char *buf, uint32_t ch, uint32_t *p_len, uint32_t reg_data)
{
	...
	if (((reg_data >> AW9620X_AFECFG3_CVOFF2X) & 0x1) == 0) {
		...
		*p_len += snprintf(buf + *p_len, PAGE_SIZE - *p_len, "ch%u parasitic_data %d.%d pf\n", ch, parasitic_data_int, parasitic_data_float);
	} else {
		*p_len += snprintf(buf + *p_len, PAGE_SIZE - *p_len, "ch%u Mutual tolerance mode does not allow 2x to be configured", ch);
	}
}
					...
					char *buf, uint32_t ch, uint32_t *p_len, uint32_t reg_data)
{
	...
	*p_len += snprintf(buf + *p_len, PAGE_SIZE - *p_len, "ch%u parasitic_data %u.%u pf\n", ch, cfg_int, cfg_float);
}
...
#define AW9620X_MODE_SET_ARR_LEN	(ARRAY_SIZE(g_aw9620x_mode_set))
...
static const struct aw_sar_para_load_t g_aw9620x_reg_arr_para = {
	...
	.reg_arr_len = ARRAY_SIZE(aw9620x_reg_default),
};
...
static const struct aw_sar_reg_list_t g_aw9620x_reg_list = {
	...
	.reg_num = ARRAY_SIZE(g_aw9620x_reg_access),
};
...
static void aw9620x_power_on_prox_detection(void *data, uint8_t en_flag)
{
	uint8_t ch = 0;
	struct aw_sar *p_sar = (struct aw_sar *)data;
	struct aw9620x *aw9620x = (struct aw9620x *)p_sar->priv_data;

	if (en_flag == true) {
		for (ch = 0; ch < AW9620X_CHANNEL_NUM_MAX; ch++) {
			aw_sar_i2c_read(p_sar->i2c,
				REG_BLFILT_CH0 + (REG_BLFILT_CH1 - REG_BLFILT_CH0) * ch,
				&(aw9620x->last_blfilta[ch]));
			aw_sar_i2c_write_bits(p_sar->i2c,
				REG_BLFILT_CH0 + (REG_BLFILT_CH1 - REG_BLFILT_CH0) * ch,
				~(0x3f << 13), (1 << 13));
		}
		aw_sar_i2c_read(p_sar->i2c, REG_IRQEN, &aw9620x->last_irq_en);
		aw_sar_i2c_write_bits(p_sar->i2c, REG_IRQEN, ~(1 << 3), 1 << 3);
	} else if (en_flag == false) {
		for (ch = 0; ch < AW9620X_CHANNEL_NUM_MAX; ch++) {
			aw_sar_i2c_write(p_sar->i2c,
				REG_BLFILT_CH0 + (REG_BLFILT_CH1 - REG_BLFILT_CH0) * ch, aw9620x->last_blfilta[ch]);
		}
		aw_sar_i2c_write(p_sar->i2c, REG_IRQEN, aw9620x->last_irq_en);
	}
}

static const struct aw_sar_power_on_prox_detection_t g_aw9620x_power_on_prox_detection = {
	.p_power_on_prox_detection_en_fn = aw9620x_power_on_prox_detection,
	.irq_en_cali_bit = 3,
...
static const struct aw_sar_chip_config g_aw9620x_chip_config = {
	...
	.power_on_prox_detection = &g_aw9620x_power_on_prox_detection,
};
...
int32_t aw9620x_init(struct aw_sar *p_sar)
{
	...
	p_sar->priv_data = devm_kzalloc(p_sar->dev, sizeof(struct aw9620x), GFP_KERNEL);
