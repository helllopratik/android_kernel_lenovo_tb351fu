...
#define AW9610X_DRIVER_VERSION "v2.4.0"
...
#endif
			;
		//AWLOGE(p_sar->dev, "curr_state[%d] = 0x%x", i, curr_status);
...
int32_t aw9610x_check_chipid(void *data)
{
	...
	if (reg_val == AW9610X_CHIP_ID) {
		...
		return AW_OK;
	} else if (reg_val != AW9610X_CHIP_ID) {
...
static void aw9610x_chip_info_get(void *data, char *buf, ssize_t *p_len)
{
	...
	*p_len += snprintf(buf + *p_len, PAGE_SIZE - *p_len, "sar%u, aw9610x chip driver version %s\n", p_sar->dts_info.sar_num, AW9610X_DRIVER_VERSION);
...
static int32_t aw9610x_awrw_read_seq(struct aw_sar *p_sar, uint8_t *reg_data)
{
	...
	for (msg_idx = 0; msg_idx < addr_bytes; msg_idx++) {
		w_buf[msg_idx] = aw9610x->aw_i2c_package.init_addr[msg_idx];
		AWLOGD(p_sar->dev, "w_buf_addr[%d] = 0x%02x", msg_idx, w_buf[msg_idx]);
	}
	...
	if (en_flag == true) {
		...
		for (ch = 0; ch < AW9610X_CHANNEL_MAX; ch++) {
			aw_sar_i2c_write(p_sar->i2c,
				REG_BLFILT_CH0 + (REG_BLFILT_CH1 - REG_BLFILT_CH0) * ch, aw9610x->last_blfilta[ch]);
		}
