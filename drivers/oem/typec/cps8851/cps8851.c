...
static int rt1711_read_device(void *client, u32 reg, int len, void *dst)
{
	...
	int ret = 0, count = 5;
	u64 t1 = 0, t2 = 0;
	cpu_latency_qos_update_request(&qos_request, 150);
	while (1) {
		t1 = local_clock();
	...
	cpu_latency_qos_update_request(&qos_request, PM_QOS_DEFAULT_VALUE);
	return ret;
}
...
#if CPS8851_A1_IDLE_FIX
static int cps8851_tcpc_sdac_save(struct rt1711_chip *chip)
{
	int sdac, tcpc;
	sdac = rt1711_i2c_read8(chip->tcpc, CPS8851_REG_PD_SDAC);
	tcpc = rt1711_i2c_read8(chip->tcpc, CPS8851_REG_PD_TCPC);
	if (sdac < 0 || tcpc < 0)
		return -EIO;
	chip->pd_sdac = (u8)sdac;
	chip->pd_tcpc = (u8)tcpc;
	return 0;
}
static int inline cps8851_tcpc_sdac_restore(struct rt1711_chip *chip)
{
	int ret;
	ret = rt1711_i2c_write8(chip->tcpc, CPS8851_REG_PASSWORD, 0x51);
	ret += rt1711_i2c_write8(chip->tcpc, CPS8851_REG_PD_SDAC, chip->pd_sdac);
	ret += rt1711_i2c_write8(chip->tcpc, CPS8851_REG_PD_TCPC, chip->pd_tcpc);
	ret += rt1711_i2c_write8(chip->tcpc, CPS8851_REG_PASSWORD, 0x00);
	return ret;
}
#endif /* CPS8851_A1_IDLE_FIX */
static int rt1711_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	mdelay(1);
#if CPS8851_A1_IDLE_FIX
	if (cps8851_tcpc_sdac_save(chip) < 0)
		dev_err(chip->dev, "%s save tcpc_sdac failed\n", __func__);
#endif
	return 0;
}
...
static int cps8851_set_rx_enable(struct tcpc_device *tcpc, uint8_t enable)
{
	...
	int ret = 0;
#if CPS8851_A1_IDLE_FIX
	if (enable != PD_RX_CAP_IDLE_MODE) {
		cps8851_tcpc_sdac_restore(chip);
	}
#endif
	...
	if (enable == PD_RX_CAP_IDLE_MODE)
	{
		cps8851_tcpc_sdac_save(chip);
		ret += rt1711_i2c_write8(tcpc, CPS8851_REG_PASSWORD, 0x51);
		ret += rt1711_i2c_write8(tcpc, CPS8851_REG_PD_TCPC, 0x10);
		ret += rt1711_i2c_write8(tcpc, CPS8851_REG_PD_SDAC, 0x00);
...
static int rt1711_set_rx_enable(struct tcpc_device *tcpc, uint8_t enable)
{
	...
	struct rt1711_chip *chip = tcpc_get_dev_data(tcpc);
	if (chip_is_cps8851(chip))
		return cps8851_set_rx_enable(tcpc, enable);
	if (enable)
