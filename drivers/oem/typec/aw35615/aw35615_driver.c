...
static int aw35615_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	chip->qos_request = kzalloc(sizeof(struct pm_qos_request), GFP_KERNEL);
	if (!chip->qos_request)
		return -ENOMEM;

	cpu_latency_qos_add_request(chip->qos_request, PM_QOS_DEFAULT_VALUE);
	...
	schedule_delayed_work(&chip->init_delay_work, msecs_to_jiffies(300));
