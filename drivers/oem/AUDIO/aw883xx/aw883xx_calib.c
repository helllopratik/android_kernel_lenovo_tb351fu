...
//TN Begin modify To audio hal read/write cali data. by lianjie.chen 20231204 OCALLA-384.
static unsigned int g_cali_re[8] = {0};
//TN End modify To audio hal read/write cali data. by lianjie.chen 20231204 OCALLA-384.

/******************************cali re store example start***********************************/
#ifdef AW_CALI_STORE_EXAMPLE
/*write cali to persist file example*/
// TN Begin modify To save result for PA calibration. by lianjie.chen 20231117 TIAIAA-8.
#define AWINIC_CALI_FILE  "/mnt/vendor/nvcfg/audio_calibration_data/aw_cali_data"
// TN End modify To save result for PA calibration. by lianjie.chen 20231117 TIAIAA-8.
//TN Begin modify for store f0 in nv by linzhi 20240108 OCALLA-1247
#define AWINIC_CALI_F0_FILE  "/mnt/vendor/nvcfg/audio_calibration_data/aw_cali_data_f0"
//TN End modify for store f0 in nv by linzhi 20240108 OCALLA-1247
...
static int aw_cali_get_cali_f0_from_file(int32_t *cali_f0, int channel)
{
	struct file *fp = NULL;
	int f_size;
	char *buf = NULL;
	...
	set_fs(fs);
#endif
	f0_buf = skip_spaces(buf);

	if (kstrtoint(f0_buf, 10, &int_cali_f0) == 0)
		*cali_f0 = int_cali_f0;
	else
		*cali_f0 = AW_ERRO_CALI_F0_VALUE;
...
static int aw_cali_read_f0_from_nvram(int32_t *cali_f0, int32_t channel)
{
#ifdef AW_CALI_STORE_EXAMPLE
	return aw_cali_get_cali_f0_from_file(cali_f0, channel);
#else
	return 0;
#endif
}
...
static int aw_cali_svc_get_cali_re_from_bin(struct aw_cali_desc *cali_desc, uint32_t *re)
{
	...
	cali_re = g_cali_re[aw_dev->channel];
...
static int aw_cali_svc_get_cali_f0_q(struct aw_device *aw_dev)
{
	...
	//TN Begin modify for store f0 in nv by linzhi 20240108 OCALLA-1247
	ret = aw_cali_write_f0_to_nvram(cali_desc->f0, aw_dev->channel);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "write f0 falied");
	}
	//TN End modify for store f0 in nv by linzhi 20240108 OCALLA-1247
						...
						bool is_enable)
{
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		//TN Begin modify to optimize cali by linzhi 20240220 OCALLA-6230
		if (local_dev->cali_desc.cur_cali_index == local_dev->cali_desc.cali_seq_index) {
			if (is_enable)
		...
		//TN End modify to optimize cali by linzhi 20240220 OCALLA-6230
	}
...
static int aw_cali_svc_devs_cali_re(struct aw_device *aw_dev,
							unsigned int flag, int cali_seq_index)
{
	int ret = -1;
	struct list_head *dev_list = NULL;
	struct list_head *pos = NULL;
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		local_dev->cali_desc.cur_cali_index = cali_seq_index;
	}

	aw_dev_info(aw_dev->dev, "process cali index %d", cali_seq_index);
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		local_dev->cali_desc.cur_cali_index = aw_dev->cali_desc.cali_seq_cnt;
	}
...
static int aw_cali_svc_cali_re(struct aw_device *aw_dev, bool is_single, unsigned int flag)
{
	...
	if (is_single)
		...
		for (i = 0; i < aw_dev->cali_desc.cali_seq_cnt; i++) {
			ret = aw_cali_svc_devs_cali_re(aw_dev, flag, i);
		}
	}
	return ret;
}
...
static int aw_cali_svc_devs_cali_f0_q(struct aw_device *aw_dev, unsigned int flag, int cali_seq_index)
{
	int ret = -1;
	struct list_head *dev_list = NULL;
	struct list_head *pos = NULL;
...
static int aw_cali_svc_cali_f0_q(struct aw_device *aw_dev, bool is_single, unsigned int flag)
{
	...
	if (is_single)
		...
		for (i = 0; i < aw_dev->cali_desc.cali_seq_cnt; i++) {
				ret = aw_cali_svc_devs_cali_f0_q(aw_dev, flag, i);
		}
	}
	return ret;
}
...
//TN Begin modify for factory tool test by linzhi 20231213 OCALLA-491
static ssize_t re_value_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct aw883xx *aw883xx = dev_get_drvdata(dev);
	struct aw_device *aw_dev = aw883xx->aw_pa;
	ssize_t len = 0;
	uint32_t thousands_place = (aw_dev->cali_desc.cali_re) / 1000;
	uint32_t hundreds_place = (aw_dev->cali_desc.cali_re) % 1000 / 100;
	uint32_t tens_place = (aw_dev->cali_desc.cali_re) % 100 / 10;
	uint32_t ones_place = (aw_dev->cali_desc.cali_re) % 10;
...
//TN Begin modify for store f0 in nv by linzhi 20240108 OCALLA-1247
	if (aw_dev->cali_desc.f0 == 0) {
		aw_cali_read_f0_from_nvram(&aw_dev->cali_desc.f0, aw_dev->channel);
	}
//TN End modify for store f0 in nv by linzhi 20240108 OCALLA-1247
	len += snprintf(buf+len, PAGE_SIZE-len, "%u\n", aw_dev->cali_desc.f0);
	return len;
}
//TN End modify for factory tool test by linzhi 20231213 OCALLA-491

//TN Begin modify for smartpa mmi&cqa test. by lianjie.chen 20231121 TIAIAA-8
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf != NULL) {
		sscanf(buf, "%d %d %d %d %d %d %d %d", &cali_data[0], &cali_data[1], &cali_data[2], &cali_data[3],
							&cali_data[4], &cali_data[5], &cali_data[6], &cali_data[7]);
	} else {
		aw_dev_err(aw_dev->dev, "write null buffer");
		return ret;
	}
	ret = aw883xx_dev_get_list_head(&dev_list);
	if (ret) {
	...
	if (dev_list != NULL) {
		list_for_each(pos, dev_list) {
			local_dev = container_of(pos, struct aw_device, list_node);
			if (local_dev != NULL) {
				//TN Begin modify for code alarm reset by linzhi 20240104 OCALLA-1114
				if (local_dev->channel < AW_DEV_CH_NUM) {
					g_cali_re[local_dev->channel] = cali_data [local_dev->channel];
					aw_dev_err(aw_dev->dev, "store cali re dev%d %d", local_dev->channel, g_cali_re[local_dev->channel]);
				}
				//TN End modify for code alarm reset by linzhi 20240104 OCALLA-1114
			}
		}
	}
	return count;
...
//TN End modify for store f0 in nv by linzhi 20240108 OCALLA-1247
//TN Begin modify for smartpa cqa cali cmd test. by lianjie.chen 20231124 OCALLA-191.
	len += snprintf(buf+len, PAGE_SIZE-len, "dev[%u]: %uHz\n",
			aw_dev->channel, aw_dev->cali_desc.f0);
//TN End modify for smartpa cqa cali cmd test. by lianjie.chen 20231124 OCALLA-191.
	return len;
}
//TN End modify for smartpa mmi&cqa test. by lianjie.chen 20231121 TIAIAA-8
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (is_single_cali) {
		...
		if (ret <= 0) {
			...
			for (i = 0; i < ret; i++)
// TN Begin modify To Cali PA. by lianjie.chen 20231228 OCALLA-891.
				len += snprintf(buf+len, PAGE_SIZE-len, "      %u", re[i]);
// TN End modify To Cali PA. by lianjie.chen 20231228 OCALLA-891.
			len += snprintf(buf+len, PAGE_SIZE-len, "\n");
		}
	}
...
//TN End modify for smartpa mmi&cqa test. by lianjie.chen 20231121 TIAIAA-8
//TN Begin modify for factory tool test by linzhi 20231213 OCALLA-491
static DEVICE_ATTR_RO(re_value);
...
//TN Begin modify to optimize cali by linzhi 20240220 OCALLA-6230
static void aw883xx_parse_cali_seq(struct aw_device *aw_dev)
{
	int ret = -1;
	struct aw_cali_desc *desc = &aw_dev->cali_desc;
	struct device_node *np = aw_dev->dev->of_node;

	ret = of_property_read_u32(np, "cali-seq-num", &desc->cali_seq_cnt);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev,
			"read cali_seq_cnt faileds");
		goto error;
	} else {
		aw_dev_info(aw_dev->dev,
			"cali_seq_cnt is %d", desc->cali_seq_cnt);
	}

	ret = of_property_read_u32(np, "cali-seq-index", &desc->cali_seq_index);
	if (ret < 0) {
		aw_dev_info(aw_dev->dev,
			"read cali-seq-index failed,default async loading fw");
		goto error;

	} else {
		aw_dev_info(aw_dev->dev,
			"cali_seq_index %d", desc->cali_seq_index);
	}

	if (desc->cali_seq_index >= desc->cali_seq_cnt ) {
		aw_dev_err(aw_dev->dev,
				"index Cannot be greater than cnt, use defalut");
		goto error;
	}

	desc->cur_cali_index  = desc->cali_seq_index;
	return;
error:
	desc->cali_seq_cnt = 1;
	desc->cali_seq_index = 0;
}
//TN End modify to optimize cali by linzhi 20240220 OCALLA-6230
