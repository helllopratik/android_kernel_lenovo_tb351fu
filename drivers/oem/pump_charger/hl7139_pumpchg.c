...
#define HL7139_REG_IBATADC1	0x44
#define HL7139_REG_IBATADC0	0x45
...
{
	return HL7139_val_toreg(3500000, 4600000, 10000, uV, false);
}

static u8 HL7139_ibatocp_toreg(u32 uA)
{
	return HL7139_val_toreg(2000000, 6600000, 100000, uA, true);
}
...
{
	return HL7139_val_toreg(4000000, 11700000, 1000000, uV, true);
}

static u8 HL7139_ibusocp_toreg(u32 uA)
{
	return HL7139_val_toreg(1000000, 3500000, 50000, uA, true);
}

static u8 HL7139_ibusocp_alm_toreg(u32 uA)
{
	return HL7139_val_toreg(0, 3500000, 50000, uA, true);
}
...
{
	bool ucpf = !!(chip->stat & BIT(HL7139_IRQIDX_CUR_I));
...
{
	dev_info(chip->dev, "%s %d\n", __func__,
		 !!(chip->stat & BIT(HL7139_IRQIDX_CUR_I)));
	return 0;
}
...
{
	dev_info(chip->dev, "%s %d\n", __func__,
		 !!(chip->stat & BIT(HL7139_STAT_CHG_I)));
	return 0;
}
...
static int HL7139_track_ov_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_track_uv_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_pmidqualsts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_vacuvp_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_ibusucp_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_fet_short_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_cfly_short_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_thermol_shutdown_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_qrb_fault_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_vbus_ovp_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_vbus_uvp_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_track_ov_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_track_uv_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_vbat_ovp_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_vout_ovp_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_vbus_pd_sts_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}

static int HL7139_vbusuvp_irq_handler(struct HL7139_chip *chip)
{
	dev_info(chip->dev, "%s\n", __func__);
	return 0;
}
...
//Antaiui <AI_BSP_CHG> <hehl> <2021-03-01> modify For 2005R 30w fast charger end
	...
	HL7139_IRQ_DESC_RS(vbusovp, HL7139_SF_A, 7, VIN_OVP_STS),
	HL7139_IRQ_DESC_RS(vbusuvp, HL7139_SF_A, 6, VIN_UVLO_STS),
	HL7139_IRQ_DESC_RS(track_ov, HL7139_SF_A, 5, TRACK_OV_STS),
	HL7139_IRQ_DESC_RS(track_uv, HL7139_SF_A, 4, TRACK_UV_STS),

	HL7139_IRQ_DESC_RS(vbatovp, HL7139_SF_A, 3, VBAT_OVP_STS),
	HL7139_IRQ_DESC_RS(voutovp, HL7139_SF_A, 2, VOUT_OVP_STS),
	HL7139_IRQ_DESC_RS(pmidqualsts, HL7139_SF_A, 1, PMID_QUAL_STS),
	HL7139_IRQ_DESC_RS(vacuvp, HL7139_SF_A, 0, VBUS_UV_STS),

	HL7139_IRQ_DESC_RS(ibusocp, HL7139_SF_B, 7, IIN_OCP_STS),
	HL7139_IRQ_DESC_RS(ibatocp, HL7139_SF_B, 6, IBAT_OCP_STS),
	HL7139_IRQ_DESC_RS(ibusucp, HL7139_SF_B, 5, IIN_UCP_STS),
	HL7139_IRQ_DESC_RS(fet_short_sts, HL7139_SF_B, 4, FET_SHORT_STS),

	HL7139_IRQ_DESC_RS(cfly_short_sts, HL7139_SF_B, 3, CFLY_SHORT_STS),
	HL7139_IRQ_DESC_RS(thermol_shutdown, HL7139_SF_B, 0, THSD_STS),


	HL7139_IRQ_DESC_RS(qrb_fault, HL7139_SF_C, 7, QPUMP_STS),
	HL7139_IRQ_DESC_RS(vbus_ovp_sts, HL7139_SF_C, 6, VIN_OVP_S),
	HL7139_IRQ_DESC_RS(vbus_uvp_sts, HL7139_SF_C, 5, VIN_UVLO_S),
	HL7139_IRQ_DESC_RS(track_ov_sts, HL7139_SF_C, 4, TRACK_OV_S),

	HL7139_IRQ_DESC_RS(track_uv_sts, HL7139_SF_C, 3, TRACK_UV_S),
	HL7139_IRQ_DESC_RS(vbat_ovp_sts, HL7139_SF_C, 2, VBAT_OVP_S),
	HL7139_IRQ_DESC_RS(vout_ovp_sts, HL7139_SF_C, 1, VOUT_OVP_S),
	HL7139_IRQ_DESC_RSSO(vbus_pd_sts, HL7139_SF_C, 0, VBUS_GOOD_S),
...
static void HL7139_irq_restore_standby(struct HL7139_chip *chip)
{
	u8 data = 0;

	HL7139_i2c_read8(chip, HL7139_REG_INT, &data);
	dev_err(chip->dev, "%s: data = 0x%02x\n", __func__, data);
}
...
static irqreturn_t HL7139_irq_handler(int irq, void *data)
{
	...
	chip->flag = 0;
	hl7139_dump_register();
	HL7139_irq_restore_standby(chip);
...
{
	chip->desc->chg_name = "primary_dvchg";
...
static int HL7139_init_irq(struct HL7139_chip *chip)
{
	...
	if (ret < 0) {
		...
		printk("%s request thread irq fail(%d)\n", __func__, ret);
		return ret;
	}
...
static const struct HL7139_dtprop HL7139_dtprops_u32[] = {
	HL7139_DT_VALPROP(vbatovp, HL7139_REG_VBATOVP, 0, 0xED,
			  HL7139_vbatovp_toreg, 0),
	HL7139_DT_VALPROP(vbatovp_alm, HL7139_REG_VBATOVP_ALM, 0, 0xED,
			  HL7139_vbatovp_toreg, 0),
	HL7139_DT_VALPROP(ibatocp, HL7139_REG_IBATOCP, 0, 0xAE,
			  HL7139_ibatocp_toreg, 0),
	HL7139_DT_VALPROP(ibatocp_alm, HL7139_REG_IBATOCP_ALM, 0, 0xAE,
			  HL7139_ibatocp_toreg, 0),
	HL7139_DT_VALPROP(ibatucp_alm, HL7139_REG_IBATUCP_ALM, 0, 0x7f,
			  HL7139_ibatucp_toreg, 0),
	HL7139_DT_VALPROP(vbusovp, HL7139_REG_CHGCTRL1, 0, 0x0f,
			  HL7139_vbusovp_toreg, 0),
	HL7139_DT_VALPROP(vbusovp_alm, HL7139_REG_CHGCTRL1, 0, 0x7f,
			  HL7139_vbusovp_toreg, 0),
	HL7139_DT_VALPROP(ibusocp, HL7139_REG_IBUSOCUCP, 0, 0x32,
			  HL7139_ibusocp_toreg, 0),
	HL7139_DT_VALPROP(ibusocp_alm, HL7139_REG_INTFLAG, 0, 0x32,
			  HL7139_ibusocp_alm_toreg, 0),
	HL7139_DT_VALPROP(wdt, HL7139_REG_ADCCTRL, 0, 0xEF,
			  HL7139_wdt_toreg, 0),
	HL7139_DT_VALPROP(vacovp, HL7139_REG_CHGCTRL0, 0, 0xE8,
			  HL7139_vacovp_toreg, 0),
	//HL7139_DT_VALPROP(ibat_rsense, HL7139_REG_REGCTRL, 1, 0x02, NULL, 0),
	//HL7139_DT_VALPROP(ibusucpf_deglitch, HL7139_REG_BUSDEGLH, 3, 0x08, NULL, 0),
};
...
//		goto ignore_intr;
	...
	chip->irq_gpio = devm_gpiod_get(chip->dev, "intr", GPIOD_IN);
...
/*	__HL7139_i2c_write8(chip,HL7139_REG_VBAT_REG , 0x7C);  //register 0x08
	...
	__HL7139_i2c_write8(chip,0x12,0x07);//modify the max value for iin ucp
	...
	__HL7139_i2c_write8(chip,0x02,0x40);
...
//Antaiui <AI_BSP_CHG> <hehl> <2021-06-09> 9v drop to 5v end
	...
	__HL7139_i2c_write8(chip,0x0F,0xE0);
			    ...
			    const struct i2c_device_id *id)
{
	...
	u8 chip_rev;
	enum HL7139_type type = HL7139_TYPE_STANDALONE;
	...
	if (IS_ERR(chip->notify_task)) {
		...
		printk("%s run notify thread fail(%d)\n",__func__, ret);
