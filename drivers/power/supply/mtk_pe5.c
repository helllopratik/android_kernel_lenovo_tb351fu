...
// TN modified by wangyang 20231123 TIAIAA-7 begin
#define PE50_DVCHG_STARTUP_CONVERT_RATIO	225	/* % */
// TN modified by wangyang 20231123 TIAIAA-7 end
...
// TN modified by wangyang 20231110 TIAIAA-7 begin
#define PE50_VBUS_CALI_THRESHOLD	200	/* mV */
// TN modified by wangyang 20231110 TIAIAA-7 end
// TN modified by liliangwu 20231215 OCALLA-336 begin
#define PE50_CV_LOWER_BOUND_GAP		10	/* mV */
...
// TN modified by wangyang 20231215 OCALLA-168 begin
#define TA_CV_ITA_ABNORMAL_THRES_MA      100
// TN modified by wangyang 20231215 OCALLA-168 end

// TN modified by liliangwu 20231215 OCALLA-336 begin
#define PE50_SUB_BUCK_CHARGE_CURRENT         1000        /* mA */
#define PE50_SUB_BUCK_INPUT_CURRENT          500         /* mA */
#define PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP 200         /* mA */
#define PE50_SUB_BUCK_INPUT_UPPER_BOUND_GAP  100         /* mA */
#define PE50_SUB_BUCK_CV                     4800000     /* uV */
#define PE50_SUB_BUCK_START_VOLTAGE          4250        /* mV */
#define PE50_PUMP_MAX_CHARGE_CURRENT         7000        /* mA */
#define PE50_PUMP_MAX_INPUT_CURRENT          3500        /* mA */
#define PE50_ITA_GAP_PRE_VSTEP               50          /* mA */

#define FFC_CV_VOLTAGE1                      4250
#define FFC_CV_VOLTAGE2                      4520
#define FFC_LIMIT_CURRENT1                   9000
#define FFC_LIMIT_CURRENT2                   7560
#define FFC_CV_VOLTAGE_GAP                   10
#define FFC_LIMIT_CURRENT_GAP                40
// TN modified by liliangwu 20231215 OCALLA-336 end

// TN modified by wangyang 20240120 OCALLA-3158 begin
#define PE50_STOP_5V_SUPPLY_VINDPM_UV        4700000
// TN modified by wangyang 20240120 OCALLA-3158 end
...
// TN modified by liliangwu 20231215 OCALLA-336 begin
	ret = pe50_get_adc(info, PE50_ADCCHAN_IBUS, ita);
	if (ret < 0) {
		PE50_ERR("get ibus fail(%d)\n", ret);
		return ret;
	}
	if (data->sub1_input_current_limit != 0)
		*ita += (data->sub1_input_current_limit - PE50_SUB_BUCK_INPUT_UPPER_BOUND_GAP);
	if (data->sub2_input_current_limit != 0)
		*ita += (data->sub2_input_current_limit - PE50_SUB_BUCK_INPUT_UPPER_BOUND_GAP);
	return ret;
// TN modified by liliangwu 20231215 OCALLA-336 end
}
...
// TN modified by liliangwu 20240116 OCALLA-2381 begin
	if (PE50_ITA_GAP_PRE_VSTEP) {
		data->ita_gap_per_vstep = PE50_ITA_GAP_PRE_VSTEP;
		return;
	}
// TN modified by liliangwu 20240116 OCALLA-2381 end
				     ...
				     u32 ita)
{
	...
	while (true) {
		...
		if (ret < 0) {
			PE50_ERR("set ta cap fail(%d)\n", ret);
// TN modified by wangyang 20240302 OCALLA-6551 begin
			data->run_once = false;
// TN modified by wangyang 20240302 OCALLA-6551 end
			return ret;
		}
...
// TN modified by liliangwu 20240119 OCALLA-2952 begin
static inline void pe50_select_vbat_cv(struct pe50_algo_info *info)
{
	...
	if (data->ffc_cv_limit > 0)
		cv_no_ircmp = min(cv_no_ircmp, (u32)data->ffc_cv_limit);
...
out:
	PE50_INFO("vbat_cv(org,limit,ffc,no_ircmp,low_bound)=%d(%d,%d,%d,%d,%d)\n",
		  data->vbat_cv, desc->vbat_cv, data->cv_limit, data->ffc_cv_limit,
		  data->vbat_cv_no_ircmp, data->cv_lower_bound);
	mutex_unlock(&data->ext_lock);
}

static inline void pe50_select_charging_current_limit(struct pe50_algo_info *info)
{
	struct pe50_algo_data *data = info->data;

	mutex_lock(&data->ext_lock);
	if (data->ext_charging_current_limit == -1 ||
		data->ffc_charging_current_limit < data->ext_charging_current_limit) {
		data->charging_current_limit = data->ffc_charging_current_limit;
	} else {
		data->charging_current_limit = data->ext_charging_current_limit;
	}
	PE50_INFO("charging_current_limit(ffc,ext)=%d(%d,%d)\n",
		  data->charging_current_limit, data->ffc_charging_current_limit,
		  data->ext_charging_current_limit);
	mutex_unlock(&data->ext_lock);
}

static inline void pe50_select_ffc(struct pe50_algo_info *info)
{
	int ret, vbat, ibat;
	struct pe50_algo_data *data = info->data;
	...
	ret = pe50_hal_get_ibat(info->alg, &ibat);
	if (ret < 0) {
		PE50_ERR("get ibat fail(%d)\n", ret);
		return;
	}
	...
	mutex_lock(&data->ext_lock);
	if (data->ffc_cv_limit == FFC_CV_VOLTAGE2 ||
		(ibat < (FFC_LIMIT_CURRENT2 + FFC_LIMIT_CURRENT_GAP) &&
		vbat > (FFC_CV_VOLTAGE1 - FFC_CV_VOLTAGE_GAP))) {
		data->ffc_cv_limit = FFC_CV_VOLTAGE2;
		data->ffc_charging_current_limit = FFC_LIMIT_CURRENT2;
	} else {
		data->ffc_cv_limit = FFC_CV_VOLTAGE1;
		data->ffc_charging_current_limit = FFC_LIMIT_CURRENT1;
	}
	PE50_INFO("ffc(cv,lmt)=(%d,%d)\n", data->ffc_cv_limit, data->ffc_charging_current_limit);
	mutex_unlock(&data->ext_lock);
}
// TN modified by liliangwu 20240119 OCALLA-2952 end
...
// TN modified by liliangwu 20231201 OCALLA-308 begin
static int pe50_start_sub1_charging(struct pe50_algo_info *info)
{
	int ret;
	PE50_INFO("start sub1\n");

	info->data->sub1_input_current_limit = PE50_SUB_BUCK_INPUT_CURRENT;
	info->data->sub1_charging_current_limit = PE50_SUB_BUCK_CHARGE_CURRENT;
	ret = pe50_hal_enable_charging(info->alg, SUB1, true);
	if (ret < 0) {
		PE50_ERR("enable sub1 fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_enable_hz(info->alg, SUB1, false);
	if (ret < 0) {
		PE50_ERR("disable sub1 hz fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_set_aicr(info->alg, SUB1,
		info->data->sub1_input_current_limit);
	if (ret < 0) {
		PE50_ERR("set sub1 aicr fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_set_ichg(info->alg, SUB1,
		info->data->sub1_charging_current_limit);
	if (ret < 0) {
		PE50_ERR("set sub1 ichg fail(%d)\n", ret);
		return ret;
	}
...
static int pe50_start_sub2_charging(struct pe50_algo_info *info)
{
	int ret;
	PE50_INFO("start sub2\n");

	info->data->sub2_input_current_limit = PE50_SUB_BUCK_INPUT_CURRENT;
	info->data->sub2_charging_current_limit = PE50_SUB_BUCK_CHARGE_CURRENT;
	ret = pe50_hal_enable_hz(info->alg, SUB2, false);
	if (ret < 0) {
		PE50_ERR("disable sub2 hz fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_enable_charging(info->alg, SUB2, true);
	if (ret < 0) {
		PE50_ERR("enable sub2 fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_set_aicr(info->alg, SUB2,
		info->data->sub2_input_current_limit);
	if (ret < 0) {
		PE50_ERR("set sub2 aicr fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_set_ichg(info->alg, SUB2,
		info->data->sub2_charging_current_limit);
	if (ret < 0) {
		PE50_ERR("set sub2 ichg fail(%d)\n", ret);
		return ret;
	}
...
static int pe50_disable_sub1_charging(struct pe50_algo_info *info)
{
	int ret;
	PE50_INFO("disable sub1\n");

	info->data->sub1_input_current_limit = 0;
	info->data->sub1_charging_current_limit = 0;
	ret = pe50_hal_enable_hz(info->alg, SUB1, true);
	if (ret < 0) {
		PE50_ERR("enable sub1 hz fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_enable_charging(info->alg, SUB1, false);
	if (ret < 0) {
		PE50_ERR("disable sub1 fail(%d)\n", ret);
		return ret;
	}
...
static int pe50_disable_sub2_charging(struct pe50_algo_info *info)
{
	int ret;
	PE50_INFO("disable sub2\n");

	info->data->sub2_input_current_limit = 0;
	info->data->sub2_charging_current_limit = 0;
	ret = pe50_hal_enable_hz(info->alg, SUB2, true);
	if (ret < 0) {
		PE50_ERR("enable sub2 hz fail(%d)\n", ret);
		return ret;
	}
	ret = pe50_hal_enable_charging(info->alg, SUB2, false);
	if (ret < 0) {
		PE50_ERR("disable sub2 fail(%d)\n", ret);
		return ret;
	}
...
static int pe50_enable_subchg_charging(struct pe50_algo_info *info, bool en)
{
	PE50_INFO("en = %d\n", en);
	if (en) {
		pe50_start_sub1_charging(info);
		pe50_start_sub2_charging(info);
	} else {
		pe50_disable_sub1_charging(info);
		pe50_disable_sub2_charging(info);
	}

	return 0;
}

static int pe50_subchg_init(struct pe50_algo_info *info)
{
	int ret;

	ret = pe50_hal_set_cv(info->alg, SUB1, PE50_SUB_BUCK_CV);
	if (ret < 0) {
		PE50_ERR("set sub1 cv fail(%d)\n", ret);
		return ret;
	}

	ret = pe50_hal_set_cv(info->alg, SUB2, PE50_SUB_BUCK_CV);
	if (ret < 0) {
		PE50_ERR("set sub2 cv fail(%d)\n", ret);
		return ret;
	}

	return 0;
}
// TN modified by liliangwu 20231201 OCALLA-308 end
...
// TN modified by liliangwu 20231227 OCALLA-873 begin
	pe50_hal_dump_registers(info->alg, CHG1);
	pe50_hal_dump_registers(info->alg, SUB1);
	pe50_hal_dump_registers(info->alg, SUB2);
// TN modified by liliangwu 20231227 OCALLA-873 end
...
// TN modified by liliangwu 20231201 OCALLA-308 begin
	pe50_enable_subchg_charging(info, false);
// TN modified by liliangwu 20231201 OCALLA-308 end
	pe50_enable_dvchg_charging(info, PE50_DVCHG_SLAVE, false);
	pe50_set_dvchg_charging(info, false);
	if (!(data->notify & PE50_RESET_NOTIFY)) {
// TN modified by wangyang 20240120 OCALLA-3158 begin
		pe50_hal_set_mivr(info->alg, CHG1, PE50_STOP_5V_SUPPLY_VINDPM_UV);
		pe50_hal_set_mivr(info->alg, SUB1, PE50_STOP_5V_SUPPLY_VINDPM_UV);
		pe50_hal_set_mivr(info->alg, SUB2, PE50_STOP_5V_SUPPLY_VINDPM_UV);
// TN modified by wangyang 20240120 OCALLA-3158 end
	}
// TN modified by liliangwu 20240227 OCALLA-6451 begin
	pe50_hal_set_ta_cap(info->alg, 9000, PE50_ITA_INIT);
// TN modified by liliangwu 20240227 OCALLA-6451 end
...
// TN modified by liliangwu 20231228 OCALLA-915 begin
	ret = pe50_enable_subchg_charging(info, false);
	if (ret < 0) {
		PE50_ERR("disable subchg fail(%d)\n", ret);
		goto start;
	}
// TN modified by liliangwu 20231228 OCALLA-915 end
...
// TN modified by liliangwu 20231215 OCALLA-336 begin
	data->ffc_cv_limit = FFC_CV_VOLTAGE1;
	data->ffc_charging_current_limit = FFC_LIMIT_CURRENT1;
	data->ext_charging_current_limit = -1;
	data->charging_current_limit = -1;
	data->sub1_input_current_limit = 0;
	data->sub1_charging_current_limit = 0;
	data->sub2_input_current_limit = 0;
	data->sub2_charging_current_limit = 0;
// TN modified by liliangwu 20231215 OCALLA-336 end
	data->cv_limit = -1;
	mutex_unlock(&data->ext_lock);
...
// TN modified by liliangwu 20231201 OCALLA-308 begin
	ret = pe50_subchg_init(info);
	if (ret < 0) {
		PE50_ERR("init subchg fail(%d)\n", ret);
		goto out;
	}
...
static int pe50_ta_cv_abnormal_handle(struct pe50_algo_info *info);
...
// TN modified by liliangwu 20231201 OCALLA-308 begin
	int bat_vbat, bat_ibat, dvchg_ibus;
	ret = pe50_hal_get_vbat(info->alg, &bat_vbat);
	if (ret < 0) {
		PE50_ERR("get bat_vbat fail(%d)\n", ret);
		goto out;
	}

	idvchg_lmt = pe50_get_idvchg_lmt(info);
	if (idvchg_lmt > (PE50_PUMP_MAX_INPUT_CURRENT +
		PE50_SUB_BUCK_INPUT_CURRENT - PE50_SUB_BUCK_INPUT_UPPER_BOUND_GAP) &&
		bat_vbat < PE50_SUB_BUCK_START_VOLTAGE && (data->charging_current_limit == -1 ||
		data->charging_current_limit > (PE50_PUMP_MAX_CHARGE_CURRENT +
		PE50_SUB_BUCK_CHARGE_CURRENT - PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP))) {

		ret = pe50_set_ta_cap_cv(info, data->vta_setting, data->ita_setting +
			(PE50_SUB_BUCK_INPUT_CURRENT << 1));
		if (ret < 0) {
			PE50_ERR("set ta cap fail(%d)\n", ret);
			sinfo.hardreset_ta = true;
			goto out;
		...
		ret = pe50_enable_subchg_charging(info, true);
		if (ret < 0) {
			PE50_ERR("enable subchg fail(%d)\n", ret);
			goto out;
		}
	}

	PE50_INFO("bat_vbat=%d idvchg_lmt=%d chg_lmt= %d\n",
		bat_vbat, idvchg_lmt, data->charging_current_limit);
// TN modified by liliangwu 20231201 OCALLA-308 end
...
// TN modified by liliangwu 20240119 OCALLA-2952 begin
	pe50_select_ffc(info);
	pe50_select_charging_current_limit(info);
...
// TN modified by liliangwu 20231201 OCALLA-308 begin
	...
	ret = pe50_hal_get_ibat(info->alg, &bat_ibat);
	if (ret < 0) {
		PE50_ERR("get bat_ibat fail(%d)\n", ret);
		goto out;
	}

	ret = pe50_hal_get_adc(info->alg, DVCHG1, PE50_ADCCHAN_IBUS,
				&dvchg_ibus);
	if (ret < 0) {
		PE50_ERR("get dvchg ibus fail(%d)\n", ret);
		goto out;
	...
	if (dvchg_ibus < TA_CV_ITA_ABNORMAL_THRES_MA) {
		PE50_ERR("dvchg_ibus = %d, abnormal situation\n", dvchg_ibus);
		return pe50_ta_cv_abnormal_handle(info);
	}

	/* VBAT reaches CV level */
	if (bat_vbat >= data->vbat_cv) {
		if (data->ita_measure < data->idvchg_term) {
			if (data->is_dvchg_en[PE50_DVCHG_SLAVE]) {
				ret = pe50_check_slave_dvchg_off(info);
				if (ret < 0) {
...
cc_cv:
		if (data->sub1_charging_current_limit > 0 && bat_ibat < (PE50_PUMP_MAX_CHARGE_CURRENT +
			((PE50_SUB_BUCK_CHARGE_CURRENT - PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP) << 1))) {
			pe50_disable_sub1_charging(info);
		}
		if (data->sub2_charging_current_limit > 0 &&
			bat_ibat < (PE50_PUMP_MAX_CHARGE_CURRENT + PE50_SUB_BUCK_CHARGE_CURRENT -
			PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP)) {
			pe50_disable_sub2_charging(info);
		}
	...
	if (data->ita_measure + ita_gap_per_vstep > idvchg_lmt ||
		vta == auth_data->vcap_max || (data->charging_current_limit != -1 &&
		(bat_ibat + (ita_gap_per_vstep << 1)) > data->charging_current_limit))
		data->state = PE50_ALGO_CC_CV;
	else {
		vta += auth_data->vta_step;
		vta = min(vta, (u32)auth_data->vcap_max);
	...
	PE50_INFO("vbat,ibat=%d,%d vta,ita(set,meas)=(%d,%d),(%d,%d) cv = %d ita_gap_per_vstep = %d\n",
		bat_vbat, bat_ibat, data->vta_setting, data->vta_measure, data->ita_setting, data->ita_measure,
		data->vbat_cv, ita_gap_per_vstep);
	PE50_INFO("sub1(input,charge) = (%d,%d) sub2(input,charge) = (%d,%d) idvchg_lmt = %d charge_lmt = %d\n",
		data->sub1_input_current_limit, data->sub1_charging_current_limit, data->sub2_input_current_limit,
		data->sub2_charging_current_limit, idvchg_lmt, data->charging_current_limit);
// TN modified by liliangwu 20231201 OCALLA-308 end
...
out_set_cap:
	...
	if (ret < 0) {
		PE50_ERR("set ta cap fail(%d)\n", ret);
		sinfo.hardreset_ta = true;
// TN modified by liliangwu 20240201 OCALLA-5552 begin
		data->run_once = false;
// TN modified by liliangwu 20240201 OCALLA-5552 end
		goto out;
	}
...
static int pe50_ta_cv_abnormal_handle(struct pe50_algo_info *info)
{
	...
	ret = pe50_notify_hardreset_hdlr(info);
	pe50_send_notification(info, EVT_SOFTRESET, &notify);
	return ret;
}
// TN modified by wangyang 20231215 OCALLA-168 end
...
{
	int ret, vbat, ibat, dvchg_ibus, dvchg_ibat, vsys = 0;
	struct pe50_algo_data *data = info->data;
	struct pe50_ta_auth_data *auth_data = &data->ta_auth_data;
	...
	ret = pe50_hal_get_adc(info->alg, DVCHG1, PE50_ADCCHAN_IBAT,
				&dvchg_ibat);
	if (ret < 0) {
		PE50_ERR("get dvchg ibat fail(%d)\n", ret);
		goto out;
	}
...
// TN modified by wangyang 20231215 OCALLA-168 end
	...
	if (data->ita_measure <= data->idvchg_term) {
		...
		pe50_send_notification(info, EVT_FULL, &notify);
		PE50_INFO("finish PE5.0 charging\n");
		goto out;
	}
cc_cv:
	...
	PE50_INFO("vbat,ibat,dvchg_ibus,dvchg_ibat=%d,%d,%d,%d vta,ita(set,meas)=(%d,%d),(%d,%d) cv = %d ita_gap_per_vstep = %d\n",
		vbat, ibat, dvchg_ibus, dvchg_ibat, data->vta_setting, data->vta_measure, data->ita_setting, data->ita_measure,
		data->vbat_cv, ita_gap_per_vstep);
	PE50_INFO("sub1(input,charge) = (%d,%d) sub2(input,charge) = (%d,%d) idvchg_lmt = %d charge_lmt = %d\n",
		data->sub1_input_current_limit, data->sub1_charging_current_limit, data->sub2_input_current_limit,
		data->sub2_charging_current_limit, idvchg_lmt, data->charging_current_limit);
	...
	if (vbat >= data->vbat_cv) {
		...
		if (data->sub1_charging_current_limit > 0 && ibat < (PE50_PUMP_MAX_CHARGE_CURRENT +
			((PE50_SUB_BUCK_CHARGE_CURRENT - PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP) << 1))) {
			pe50_disable_sub1_charging(info);
		} else if (data->sub2_charging_current_limit > 0 &&
			ibat < (PE50_PUMP_MAX_CHARGE_CURRENT + PE50_SUB_BUCK_CHARGE_CURRENT -
			PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP)) {
			pe50_disable_sub2_charging(info);
		} else {
			vta -= auth_data->vta_step;
		...
		data->is_vbat_over_cv = true;
	} else if (data->charging_current_limit != -1 &&
		ibat > (data->charging_current_limit + (ita_gap_per_vstep << 1))) {
		PE50_INFO("--ibat > chr_lmt, %d > %d\n", ibat, data->charging_current_limit);
		if (data->charging_current_limit <= (PE50_PUMP_MAX_CHARGE_CURRENT + PE50_SUB_BUCK_CHARGE_CURRENT) &&
			data->sub1_charging_current_limit > 0 && ibat < (PE50_PUMP_MAX_CHARGE_CURRENT +
			((PE50_SUB_BUCK_CHARGE_CURRENT - PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP) << 1))) {
			pe50_disable_sub1_charging(info);
		} else if (data->charging_current_limit <= PE50_PUMP_MAX_CHARGE_CURRENT &&
			data->sub2_charging_current_limit > 0 &&
			ibat < (PE50_PUMP_MAX_CHARGE_CURRENT + PE50_SUB_BUCK_CHARGE_CURRENT -
			PE50_SUB_BUCK_CHARGE_UPPER_BOUND_GAP)) {
			pe50_disable_sub2_charging(info);
	...
	} else if (data->ita_measure > (idvchg_lmt + ita_gap_per_vstep)
		|| vsys >= PE50_VSYS_UPPER_BOUND) {
		if (idvchg_lmt <= (PE50_PUMP_MAX_INPUT_CURRENT + PE50_SUB_BUCK_INPUT_CURRENT) &&
			data->sub1_input_current_limit > 0 && data->ita_measure < (PE50_PUMP_MAX_INPUT_CURRENT +
			((PE50_SUB_BUCK_INPUT_CURRENT - PE50_SUB_BUCK_INPUT_UPPER_BOUND_GAP) << 1))) {
			pe50_disable_sub1_charging(info);
		} else if (idvchg_lmt <= PE50_PUMP_MAX_INPUT_CURRENT &&
			data->sub2_input_current_limit > 0 &&
			data->ita_measure < (PE50_PUMP_MAX_INPUT_CURRENT +
			PE50_SUB_BUCK_INPUT_CURRENT - PE50_SUB_BUCK_INPUT_UPPER_BOUND_GAP)) {
			pe50_disable_sub2_charging(info);
		} else {
			vta -= auth_data->vta_step;
		...
		if (idvchg_lmt > PE50_PUMP_MAX_INPUT_CURRENT && data->charging_current_limit >
			PE50_PUMP_MAX_CHARGE_CURRENT && vbat < PE50_SUB_BUCK_START_VOLTAGE &&
			data->vbat_cv <= PE50_SUB_BUCK_START_VOLTAGE && data->sub2_input_current_limit == 0) {
			pe50_start_sub2_charging(info);
		} else if (idvchg_lmt > (PE50_PUMP_MAX_INPUT_CURRENT + PE50_SUB_BUCK_INPUT_CURRENT) &&
			data->charging_current_limit > (PE50_PUMP_MAX_CHARGE_CURRENT + PE50_SUB_BUCK_CHARGE_CURRENT) &&
			vbat < PE50_SUB_BUCK_START_VOLTAGE && data->vbat_cv <= PE50_SUB_BUCK_START_VOLTAGE &&
			data->sub1_input_current_limit == 0) {
			pe50_start_sub1_charging(info);
		} else if (dvchg_ibus < (PE50_PUMP_MAX_INPUT_CURRENT + (PE50_SUB_BUCK_INPUT_UPPER_BOUND_GAP << 1))) {
			vta += auth_data->vta_step;
			vta = min(vta, (u32)auth_data->vcap_max);
			ita += ita_gap_per_vstep;
			ita = min(ita, idvchg_lmt);
	...
	if (data->vbat_cv > PE50_SUB_BUCK_START_VOLTAGE && data->sub1_input_current_limit != 0)
		pe50_disable_sub1_charging(info);
	if (data->vbat_cv > PE50_SUB_BUCK_START_VOLTAGE && data->sub2_input_current_limit != 0)
		pe50_disable_sub2_charging(info);
...
// TN modified by liliangwu 20240227 OCALLA-6454 begin
	return true;
// TN modified by liliangwu 20240227 OCALLA-6454 end
...
// TN modified by liliangwu 20240111 OCALLA-1470 begin
		return true;
// TN modified by liliangwu 20240111 OCALLA-1470 end
	}
	return true;
}
...
{
// TN modified by liliangwu 20231227 OCALLA-870 begin
	struct pe50_algo_data *data = info->data;
	struct pe50_stop_info sinfo = {
		.reset_ta = true,
		.hardreset_ta = false,
	...
	PE50_INFO("++\n");
	data->run_once = false;
// TN modified by liliangwu 20231227 OCALLA-870 end
	return pe50_stop(info, &sinfo);
}
...
static int pe50_stop_algo(struct chg_alg_device *alg)
{
	...
	if (!data->inited)
		goto out;
// TN modified by wangyang 20231120 OCALLA-18 begin
	data->run_once = false;
// TN modified by wangyang 20231120 OCALLA-18 end
				  ...
				  struct chg_limit_setting *setting)
{
	...
	int cc = micro_to_milli(setting->charging_current_limit_dvchg1);

	mutex_lock(&data->ext_lock);
	if (data->cv_limit != cv || data->input_current_limit != ic || data->ext_charging_current_limit != cc ) {
		data->cv_limit = cv;
		data->input_current_limit = ic;
		data->ext_charging_current_limit = cc;
		PE50_INFO("ic = %d, cv = %d, cc = %d\n", ic, cv, cc);
		pe50_wakeup_algo_thread(data);
	}
	mutex_unlock(&data->ext_lock);
	return 0;
