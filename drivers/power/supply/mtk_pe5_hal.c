...
// TN modified by liliangwu 20231201 OCALLA-308 begin
	{
		.type = MTK_CHGTYP_SUB1,
		.name = "sub_chg1",
		.must_exist = false,
	},
	{
		.type = MTK_CHGTYP_SUB2,
		.name = "sub_chg2",
		.must_exist = false,
	},
// TN modified by liliangwu 20231201 OCALLA-308 end
...
// TN modified by liliangwu 20231201 OCALLA-308 begin
	case SUB1:
		return MTK_CHGTYP_SUB1;
	case SUB2:
		return MTK_CHGTYP_SUB2;
// TN modified by liliangwu 20231201 OCALLA-308 end
	default:
		return -ENOTSUPP;
	}
...
int pe50_hal_set_cv(struct chg_alg_device *alg,
				 enum chg_idx chgidx, u32 uV)
{
	...
	if (chgtyp < 0)
		return chgtyp;
	return charger_dev_set_constant_voltage(hal->chgdevs[chgtyp], uV);
}

int pe50_hal_get_vbat(struct chg_alg_device *alg, int *vbat)
{
	int ret;
	struct power_supply *bat_psy;
	union power_supply_propval prop;
	...
	if (IS_ERR_OR_NULL(bat_psy)) {
		...
		if (ret < 0)
			return ret;
		*vbat = micro_to_milli(prop.intval);
	}
...
int pe50_hal_get_ibat(struct chg_alg_device *alg, int *ibat)
{
	int ret;
	struct power_supply *bat_psy;
	union power_supply_propval prop;
	...
	if (IS_ERR_OR_NULL(bat_psy)) {
		...
		if (ret < 0)
			return ret;
		*ibat = micro_to_milli(prop.intval);
	}
