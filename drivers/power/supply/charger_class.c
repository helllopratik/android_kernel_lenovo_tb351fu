...
// TN modified by liliangwu 20231127 OCALLA-308 begin
int charger_dev_request_hvdcp_voltage(struct charger_device *charger_dev, u8 vol)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
		charger_dev->ops->request_hvdcp_voltage)
		return charger_dev->ops->request_hvdcp_voltage(charger_dev, vol);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_request_hvdcp_voltage);

int charger_dev_is_hvdcp(struct charger_device *charger_dev)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
		charger_dev->ops->is_hvdcp)
		return charger_dev->ops->is_hvdcp(charger_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_is_hvdcp);
// TN modified by liliangwu 20231127 OCALLA-308 end

// TN modified by liliangwu 20231205 OCALLA-382 begin
int charger_dev_ship_enable(struct charger_device *charger_dev, bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
		charger_dev->ops->ship_enable)
		return charger_dev->ops->ship_enable(charger_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_ship_enable);
