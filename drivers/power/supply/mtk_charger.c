...
// TN modified by wangyang 20231206 OCALLA-195 begin
#include "../../oem/switch_charger/charge_common.h"
// TN modified by wangyang 20231206 OCALLA-195 end

// TN modified by liliangwu 20231215 OCALLA-336 begin
#define JEITA_CV_VOLTAGE                     4200
#define JEITA_LIMIT_0_TO_15_CURRENT          2520000
#define JEITA_LIMIT_15_TO_45_CURRENT         9000000
#define JEITA_LIMIT_45_TO_50_CURRENT         4200000
...
// TN modified by wangyang 20231229 OCALLA-782 begin
			strncpy(__chg_cmdline, bootargs, COMMAND_LINE_SIZE);
...
void do_sw_jeita_state_machine(struct mtk_charger *info)
{
	...
	sw_jeita->jeita_charging_current_limit = -1;
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		if ((sw_jeita->sm == TEMP_ABOVE_T4)
			...
			if (vbat < JEITA_CV_VOLTAGE) {
				sw_jeita->jeita_charging_current_limit = JEITA_LIMIT_45_TO_50_CURRENT;
			} else {
				sw_jeita->charging = false;
			}
		}
		...
		if (((sw_jeita->sm == TEMP_T3_TO_T4)
			...
			sw_jeita->jeita_charging_current_limit = JEITA_LIMIT_15_TO_45_CURRENT;
		}
		...
		if ((sw_jeita->sm == TEMP_T0_TO_T1
			...
			sw_jeita->sm = TEMP_T1_TO_T2;
			sw_jeita->jeita_charging_current_limit = JEITA_LIMIT_0_TO_15_CURRENT;
		}
	...
	chr_err("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d limit:%d\n",
		sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
		sw_jeita->cv, sw_jeita->jeita_charging_current_limit);
}
// TN modified by liliangwu 20231215 OCALLA-336 end
...
// TN modified by wangyang 20231212 OCALLA-478 begin
					       ...
					       char *buf)
{
	...
	if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO)
		...
		case POWER_SUPPLY_TYPE_USB_HVDCP:
			type_name = "HVDCP";
			break;
		case POWER_SUPPLY_TYPE_USB_OTHER:
			type_name = "FLOAT";
			break;
		case POWER_SUPPLY_TYPE_USB_DIVIDER1:
		case POWER_SUPPLY_TYPE_USB_DIVIDER2:
		case POWER_SUPPLY_TYPE_USB_DIVIDER3:
		case POWER_SUPPLY_TYPE_USB_DIVIDER4:
			type_name = "BrickID";
			break;
		default:
			type_name = "Unknown";
...
// TN modified by wangyang 20231212 OCALLA-478 end
...
// TN modified by liliangwu 20240102 OCALLA-169 begin
static ssize_t battery_present_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;

	chr_err("%s\n", __func__);
	return sprintf(buf, "%d\n", is_battery_exist(pinfo));
}

static DEVICE_ATTR_RO(battery_present);

static ssize_t battery_temp_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;

	chr_err("%s\n", __func__);
	return sprintf(buf, "%d\n", get_battery_temperature(pinfo)*10);
}

static DEVICE_ATTR_RO(battery_temp);

static ssize_t is_float_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	chr_err("%s\n", __func__);
	return sprintf(buf, "%d\n", mtk_chg_get_charger_type() == CHARGER_TYPE_FLOAT);
}

static DEVICE_ATTR_RO(is_float);
// TN modified by liliangwu 20240102 OCALLA-169 begin
...
// TN modified by liliangwu 20240118 OCALLA-2730 begin
	is_ta_detected =  (mtk_chg_get_charger_type() == CHARGER_TYPE_PPS);
// TN modified by liliangwu 20240118 OCALLA-2730 end
...
// TN modified by wangyang 20231223 OCALLA-165 begin
static void input_restore_sub_buck_config(struct mtk_charger *info)
{
	if (info->pd_type == MTK_PD_CONNECT_NONE || info->pd_type == MTK_PD_CONNECT_TYPEC_ONLY_SNK) {
		if (info->chr_type != POWER_SUPPLY_TYPE_USB && info->chr_type != POWER_SUPPLY_TYPE_USB_OTHER) {
			charger_dev_enable_hz(info->sub_chg1_dev, false);
			charger_dev_enable_hz(info->sub_chg2_dev, false);
		}
	} else {
		charger_dev_enable_hz(info->sub_chg1_dev, false);
		charger_dev_enable_hz(info->sub_chg2_dev, false);
						...
						const char *buf, size_t size)
{
	...
	if (kstrtouint(buf, 10, &val) == 0) {
		chr_err("%s: %d\n", __func__, val);
		pinfo->input_suspend = val;
		if (pinfo->input_suspend) {
			if (pinfo->chr_type != POWER_SUPPLY_TYPE_UNKNOWN) {
				charger_dev_enable_hz(pinfo->chg1_dev, true);
				charger_dev_enable_hz(pinfo->sub_chg1_dev, true);
				charger_dev_enable_hz(pinfo->sub_chg2_dev, true);
			}
		} else {
			if (chg_alg_is_algo_ready(pinfo->alg[0]) != ALG_RUNNING) {
				charger_dev_enable_hz(pinfo->chg1_dev, false);
				input_restore_sub_buck_config(pinfo);
			}
		}
		_wake_up_charger(pinfo);
	} else
...
static DEVICE_ATTR_RW(input_suspend);

static ssize_t charging_enabled_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
...
static ssize_t charging_enabled_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t size)
{
	struct mtk_charger *pinfo = dev->driver_data;
	unsigned int val;
...
// TN modified by wangyang 20231223 OCALLA-165 end

// TN modified by wangyang 20231226 OCALLA-782 begin
static ssize_t bat_maint_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;

	chr_debug("%s: %d\n", __func__, pinfo->bat_maint);
	return sprintf(buf, "%d\n", pinfo->bat_maint);
}

static ssize_t bat_maint_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t size)
{
	struct mtk_charger *pinfo = dev->driver_data;
	unsigned int val;
...
static DEVICE_ATTR_RW(bat_maint);

static ssize_t bat_maint2_show(struct device *dev, struct device_attribute *attr,
					       char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;

	chr_debug("%s: %d\n", __func__, pinfo->bat_maint2);
	return sprintf(buf, "%d\n", pinfo->bat_maint2);
}

static ssize_t bat_maint2_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t size)
{
	struct mtk_charger *pinfo = dev->driver_data;
	unsigned int val;
...
static DEVICE_ATTR_RW(bat_maint2);
// TN modified by wangyang 20231226 OCALLA-782 end

// TN modified by wangyang 20240130 OCALLA-5240 begin
static ssize_t pump_ibus_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	int val = 0, ret;

	ret = charger_dev_get_adc(pinfo->dvchg1_dev, ADC_CHANNEL_IBUS, &val, &val);
	if (ret < 0)
		return ret;
	return sprintf(buf, "%d\n", val);
}

static DEVICE_ATTR_RO(pump_ibus);
// TN modified by wangyang 20240130 OCALLA-5240 end
...
// TN modified by liliangwu 20240305 OCALLA-2931 begin
static ssize_t online_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct mtk_charger *pinfo = dev->driver_data;
	return sprintf(buf, "%d\n", is_charger_exist(pinfo));
}

static DEVICE_ATTR_RO(online);
// TN modified by liliangwu 20240305 OCALLA-2931 end
...
// TN modified by liliangwu 20231205 OCALLA-382 begin
...
static ssize_t ship_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	long val = 0;
	int ret;
	struct power_supply *chg_psy = NULL;
	...
	if (buf != NULL && size != 0) {
		chr_err("[ship mode enable] buf is %s\n", buf);
		ret = kstrtol(buf, 10, &val);
		if (ret == -ERANGE || ret == -EINVAL)
			return -EINVAL;
		info->ship_is_enable = val;
	}
	return size;
}
static DEVICE_ATTR_RW(ship_mode);
// TN modified by liliangwu 20231205 OCALLA-382 end
...
// TN modified by liliangwu 20231221 OCALLA-336 begin
int mtk_chg_get_charger_type(void)
{
	static struct mtk_charger *pinfo;
	int charger_type = CHARGER_TYPE_UNKNOW;
	struct power_supply *psy;
	...
	if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO)
		charger_type = CHARGER_TYPE_PPS;
	else if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30 ||
		pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK) {
		charger_type = pinfo->sub_pd_type;
	} else {
		switch (pinfo->chr_type) {
		case POWER_SUPPLY_TYPE_UNKNOWN:
			charger_type = CHARGER_TYPE_UNKNOW;
			break;
		case POWER_SUPPLY_TYPE_USB_OTHER:
		...
		case POWER_SUPPLY_TYPE_USB_DCP:
			...
			break;
		case POWER_SUPPLY_TYPE_USB_DIVIDER1:
			charger_type = CHARGER_TYPE_DIVIDER1;
			break;
		case POWER_SUPPLY_TYPE_USB_DIVIDER2:
			charger_type = CHARGER_TYPE_DIVIDER2;
			break;
		case POWER_SUPPLY_TYPE_USB_DIVIDER3:
			charger_type = CHARGER_TYPE_DIVIDER3;
			break;
		case POWER_SUPPLY_TYPE_USB_DIVIDER4:
			charger_type = CHARGER_TYPE_DIVIDER4;
			break;
		case POWER_SUPPLY_TYPE_USB_HVDCP:
			charger_type = CHARGER_TYPE_QC;
			break;
		default:
			charger_type = CHARGER_TYPE_UNKNOW;
			break;
...
EXPORT_SYMBOL(mtk_chg_get_charger_type);
// TN modified by liliangwu 20231221 OCALLA-336 end
...
// TN modified by liliangwu 20240102 OCALLA-169 begin
static void mtk_battery_notify_VChrType_check(struct mtk_charger *info)
{
	int type = mtk_chg_get_charger_type();
	if (type != CHARGER_TYPE_FLOAT)
		info->notify_code &= ~CHG_TYPE_FLOAT_STATUS;
	else {
		info->notify_code |= CHG_TYPE_FLOAT_STATUS;
		mtk_chgstat_notify(info);
	}
}
// TN modified by liliangwu 20240102 OCALLA-169 end
...
// TN modified by liliangwu 20240102 OCALLA-169 begin
		mtk_battery_notify_VChrType_check(info);
// TN modified by liliangwu 20240102 OCALLA-169 end
	} else {
...
// TN modified by wangyang 20231120 OCALLA-18 begin
	if (get_charger_type(info) == POWER_SUPPLY_TYPE_UNKNOWN && info->pd_type == MTK_PD_CONNECT_NONE)
		return;
// TN modified by wangyang 20231120 OCALLA-18 end
...
// TN modified by liliangwu 20231215 OCALLA-336 begin
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d limit:%d) (sm:%d) en:%d c:%d s:%d ov:%d sc:%d %d %d saf_cmd:%d bat_mon:%d %d\n",
		temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging,
		info->sw_jeita.jeita_charging_current_limit, thermal->sm,
...
// TN modified by wangyang 20231120 OCALLA-18 begin
	info->sub_chg1_dev = get_charger_by_name("sub_chg1");
	if (info->sub_chg1_dev)
		chr_err("%s, Found first sub charger\n", __func__);
	else {
		chr_err("%s, *** Error : can't find first sub charger ***\n", __func__);
		return false;
	}

	info->sub_chg2_dev = get_charger_by_name("sub_chg2");
	if (info->sub_chg2_dev)
		chr_err("%s, Found second sub charger\n", __func__);
	else {
		chr_err("%s, *** Error : can't find second sub charger ***\n", __func__);
		return false;
	}
// TN modified by wangyang 20231120 OCALLA-18 end
...
// TN modified by wangyang 20231206 OCALLA-195 begin
static void sub_buck_charger_config_plug_in(struct mtk_charger *info)
{
	if (info->pd_type == MTK_PD_CONNECT_NONE || info->pd_type == MTK_PD_CONNECT_TYPEC_ONLY_SNK) {
...
// TN modified by wangyang 20240110 OCALLA-1352 begin
			...
			charger_dev_set_mivr(info->sub_chg1_dev, HVDCP_VINDPM_UV);
			charger_dev_set_mivr(info->sub_chg2_dev, HVDCP_VINDPM_UV);
// TN modified by wangyang 20240110 OCALLA-1352 end
			charger_dev_enable(info->sub_chg1_dev, true);
			charger_dev_enable(info->sub_chg2_dev, true);
			charger_dev_enable_hz(info->sub_chg1_dev, false);
...
static void sub_buck_charger_config_plug_out(struct mtk_charger *info)
{
	charger_dev_enable(info->sub_chg1_dev, false);
	charger_dev_enable(info->sub_chg2_dev, false);
	charger_dev_enable_hz(info->sub_chg1_dev, true);
...
// TN modified by wangyang 20231206 OCALLA-195 begin
	sub_buck_charger_config_plug_out(info);
// TN modified by wangyang 20231206 OCALLA-195 end
...
// TN modified by wangyang 20231206 OCALLA-195 begin
	sub_buck_charger_config_plug_in(info);
// TN modified by wangyang 20231206 OCALLA-195 out
// TN modified by wangyang 20240117 OCALLA-1374 begin
	if (info->input_suspend) {
		charger_dev_enable_hz(info->chg1_dev, true);
		charger_dev_enable_hz(info->sub_chg1_dev, true);
		charger_dev_enable_hz(info->sub_chg2_dev, true);
	}
// TN modified by wangyang 20240117 OCALLA-1374 end
...
// TN modified by wangyang 20231110 TIAIAA-7 begin
	if (chr_type == POWER_SUPPLY_TYPE_UNKNOWN && info->pd_type == MTK_PD_CONNECT_NONE)
		return false;
...
// TN modified by liliangwu 20240116 OCALLA-2186 begin
	if (boot_mode == 9) {
// TN modified by liliangwu 20240116 OCALLA-2186 end
		vbus = get_vbus(info);
...
// TN modified by wangyang 20231109 TIAIAA-7 begin
#if IS_ENABLED(CONFIG_CHARGER_SC8989X) || IS_ENABLED(CONFIG_CHARGER_HL7015)
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		chr_err("%s Couldn't get chg_psy\n", __func__);
#endif
// TN modified by wangyang 20231109 TIAIAA-7 end
	} else {
...
// TN modified by liliangwu 20240102 OCALLA-169 begin
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_battery_temp);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_is_float);
	if (ret)
		goto _out;
// TN modified by liliangwu 20240102 OCALLA-169 end
...
// TN modified by wangyang 20231223 OCALLA-165 begin
	ret = device_create_file(&(pdev->dev), &dev_attr_input_suspend);
	if (ret)
		goto _out;
...
// TN modified by wangyang 20231226 OCALLA-782 begin
	ret = device_create_file(&(pdev->dev), &dev_attr_bat_maint);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_bat_maint2);
	if (ret)
		goto _out;
// TN modified by wangyang 20231226 OCALLA-782 end

// TN modified by wangyang 20240130 OCALLA-5240 begin
	ret = device_create_file(&(pdev->dev), &dev_attr_pump_ibus);
	if (ret)
		goto _out;
// TN modified by wangyang 20240130 OCALLA-5240 end
...
// TN modified by wangyang 20231229 OCALLA-782 begin
void mtk_charger_init_bat_maint(struct mtk_charger *info)
{
	char *ptr = NULL;

	ptr = strstr(chg_get_cmd(), "bat_maint=");
	if (ptr) {
		ptr += strlen("bat_maint=");
		if (ptr[0] == 'Y')
			info->bat_maint = true;
		else
			info->bat_maint = false;
		if (ptr[1] == 'Y')
			info->bat_maint2 = true;
		else
			info->bat_maint2 = false;
	} else
		chr_err("%s: can't find bat maint info in cmdline\n", __func__);
}
// TN modified by wangyang 20231229 OCALLA-782 end
...
// TN modified by wangyang 20231218 OCALLA-510 begin
	POWER_SUPPLY_PROP_STATUS,
// TN modified by wangyang 20231218 OCALLA-510 end
...
// TN modified by wangyang 20231218 OCALLA-510 begin
static int mtk_charger_get_status(struct mtk_charger *info)
{
	union power_supply_propval online;
	int ret;

	if (info->chg_psy) {
		ret = power_supply_get_property(info->chg_psy, POWER_SUPPLY_PROP_ONLINE, &online);
		if (ret)
		...
		if (info->can_charging && !info->cmd_discharging)
...
// TN modified by wangyang 20231218 OCALLA-510 begin
	case POWER_SUPPLY_PROP_STATUS:
		val->intval = mtk_charger_get_status(info);
		break;
// TN modified by wangyang 20231218 OCALLA-510 end
...
// TN modified by liliangwu 20240304 OCALLA-8417 begin
		if (pinfo->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			adapter_dev_set_cap(pinfo->pd_adapter, MTK_PD_APDO, 5000, 3000);
			adapter_dev_set_cap(pinfo->pd_adapter, MTK_PD_APDO_END, 5000, 3000);
		}
// TN modified by liliangwu 20240304 OCALLA-8417 end
		pinfo->pd_type = MTK_PD_CONNECT_NONE;
// TN modified by liliangwu 20240220 OCALLA-6391 begin
		if (pinfo->sub_pd_type == CHARGER_TYPE_PD_9V) {
			charger_dev_set_mivr(pinfo->chg1_dev, pinfo->data.min_charger_voltage);
			charger_dev_set_mivr(pinfo->sub_chg1_dev, pinfo->data.min_charger_voltage);
			charger_dev_set_mivr(pinfo->sub_chg2_dev, pinfo->data.min_charger_voltage);
		}
		pinfo->sub_pd_type = CHARGER_TYPE_UNKNOW;
// TN modified by liliangwu 20240220 OCALLA-6391 end
		pinfo->pd_reset = false;
		mutex_unlock(&pinfo->pd_lock);
		mtk_chg_alg_notify_call(pinfo, EVT_DETACH, 0);
		/* reset PE40 */
// TN modified by liliangwu 20231218 OCALLA-336 begin
		mutex_lock(&pinfo->charger_lock);
		pinfo->charger_thread_polling = false;
		pinfo->is_chg_done = false;
		mutex_unlock(&pinfo->charger_lock);
// TN modified by liliangwu 20231218 OCALLA-336 end
		break;
	...
	case MTK_PD_CONNECT_PE_READY_SNK:
		...
		mutex_unlock(&pinfo->pd_lock);
		/* PD is ready */
// TN modified by liliangwu 20240221 OCALLA-6462 begin
		mutex_lock(&pinfo->charger_lock);
		mtk_charger_force_disable_power_path(pinfo, CHG1_SETTING, false);
		pinfo->charger_thread_polling = true;
		mutex_unlock(&pinfo->charger_lock);
		_wake_up_charger(pinfo);
// TN modified by liliangwu 20240221 OCALLA-6462 end
		break;
	...
	case MTK_PD_CONNECT_PE_READY_SNK_PD30:
		...
		mutex_unlock(&pinfo->pd_lock);
		/* PD30 is ready */
// TN modified by liliangwu 20240202 OCALLA-2621 begin
		mutex_lock(&pinfo->charger_lock);
		mtk_charger_force_disable_power_path(pinfo, CHG1_SETTING, false);
		pinfo->charger_thread_polling = true;
		mutex_unlock(&pinfo->charger_lock);
		_wake_up_charger(pinfo);
// TN modified by liliangwu 20240202 OCALLA-2621 end
		break;
...
// TN modified by wangyang 20231215 OCALLA-168 begin
	struct mtk_charger *pinfo = container_of(notifier, struct mtk_charger, chg_alg_nb);

	chr_err("%s: evt:%lu\n", __func__, event);
	if (event == EVT_SOFTRESET)
		_wake_up_charger(pinfo);
...
// TN modified by liliangwu 20231222 OCALLA-336 begin
	if (event == EVT_FULL) {
		mutex_lock(&pinfo->charger_lock);
		pinfo->is_chg_done = true;
		mutex_unlock(&pinfo->charger_lock);
	}

	if (event == EVT_ALGO_STOP) {
		if (!pinfo->input_suspend) {
			reset_buck_to_default(pinfo);
			_wake_up_charger(pinfo);
		} else
			charger_dev_enable_hz(pinfo->chg1_dev, true);
	}
...
// TN modified by wangyang 20231108 TIAIAA-7 begin
...
// TN modified by wangyang 20231108 TIAIAA-7 end
...
// TN modified by wangyang 20231229 OCALLA-782 begin
	mtk_charger_init_bat_maint(info);
// TN modified by wangyang 20231229 OCALLA-782 end
...
// TN modified by liliangwu 20231205 OCALLA-382 begin
	if (info->ship_is_enable) {
		chr_err("%s: enable ship mode\n", __func__);
		charger_dev_ship_enable(info->sub_chg1_dev, info->ship_is_enable);
		charger_dev_ship_enable(info->sub_chg2_dev, info->ship_is_enable);
		charger_dev_ship_enable(info->chg1_dev, info->ship_is_enable);
	}
// TN modified by liliangwu 20231205 OCALLA-382 end
}
